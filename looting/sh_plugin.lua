...
if (SERVER) then
		...
		nut.inventory.instance('grid')
			:next(function(inventory)
				if (IsValid(entity)) then
					...
					local pInv = ply:getChar():getInv();
					local items = pInv:getItems()
					for _, v in pairs(items) do
						print(v.uniqueID)
						print(v:getID())
						print(tostring(entity))
						print(tostring(entity:getInv()))
						...
						if (v:getData("equip")) then
							...
							pInv:transfer(entity:getNetVar("id"))
	...
	netstream.Hook("lootOpen", function(entity, index)
		...
		if (IsValid(entity) and inventory and inventory.items and false) then
